...
async fn get_server(
    ...
    let server = state
        .config_service
        .get_server_for_user(&user.id, &id)
        .await
        .map_err(ApiError::from)?;
...
async fn add_server(
    ...
    let server = state
        .config_service
        .add_server_for_user(&user.id, req)
        .await
        .map_err(ApiError::from)?;
...
async fn edit_server(
    ...
    let server = state
        .config_service
        .edit_server_for_user(&user.id, &id, req)
        .await
        .map_err(ApiError::from)?;
    Ok(Json(ApiResponse::ok(server)))
}
...
async fn delete_server(
    ...
    state
        .config_service
        .delete_server_for_user(&user.id, &id)
        .await
        .map_err(ApiError::from)?;
...
async fn toggle_server(
    ...
    let server = state
        .config_service
        .toggle_server_for_user(&user.id, &id)
        .await
        .map_err(ApiError::from)?;
    Ok(Json(ApiResponse::ok(server)))
}
...
async fn batch_import(
    ...
    let servers = state
        .config_service
        .batch_import_for_user(&user.id, req)
        .await
        .map_err(ApiError::from)?;
    Ok(Json(ApiResponse::ok(servers)))
}
...
async fn test_connection(
    ...
    if let Some(server_id) = req.id.as_deref() {
        state
            .config_service
            .persist_test_result_for_user(&user.id, server_id, &result)
            .await
            .map_err(ApiError::from)?;
    }
...
async fn oauth_check_status(
    ...
    let status = state
        .oauth_service
        .check_oauth_status_for_user(&user.id, &req.server_url)
        .await
        .map_err(ApiError::from)?;
...
async fn oauth_login(
    ...
    let result = state
        .oauth_service
        .login_for_user(&user.id, &req.server_url)
        .await
        .map_err(ApiError::from)?;
...
async fn oauth_logout(
    ...
    state
        .oauth_service
        .logout_for_user(&user.id, &req.server_url)
        .await
        .map_err(ApiError::from)?;
    Ok(Json(ApiResponse::success()))
}
...
async fn oauth_authenticated(
    ...
    let urls = state
        .oauth_service
        .get_authenticated_servers_for_user(&user.id)
        .await
        .map_err(ApiError::from)?;
