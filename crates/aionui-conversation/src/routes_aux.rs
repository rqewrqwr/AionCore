...
async fn set_config_option(
    ...
    Ok(Json(ApiResponse::ok(
        state
            .service
            .set_config_option_for_user(&user.id, &id, &option_id, req)
            .await
...
async fn side_question(
    ...
    Ok(Json(ApiResponse::ok(
        state
            .service
            .handle_side_question_for_user(&user.id, &id, req)
            .await
            .map_err(ApiError::from)?,
    )))
}
...
async fn get_slash_commands(
    ...
    Ok(Json(ApiResponse::ok(
        state
            .service
            .get_slash_commands_for_user(&user.id, &id)
            .await
            .map_err(ApiError::from)?,
    )))
}
...
async fn browse_workspace(
    ...
    Ok(Json(ApiResponse::ok(
        state
            .service
            .browse_workspace_for_user(&user.id, &id, query)
            .await
            .map_err(ApiError::from)?,
    )))
}
